...
fabric.ShapeTwoArrow = fabric.util.createClass(fabric.Line, {
    type: 'ShapeTwoArrow',
    ...
    _render: function (ctx) {
        ...
        let xDiff = this.width;
        let yDiff = this.height;
        let angle = Math.atan2(yDiff, xDiff);

        this.ctx.save();
        this.ctx.translate(p.x1, p.y1);
...
fabric.ShapeTwoArrow.fromObject = function(object, callback) {
    const points = [object.x1, object.y1, object.x2, object.y2];
    const options = fabric.util.object.clone(object);

    // 创建新的 ShapeArrow 对象
    const shapeTwoArrow = new fabric.ShapeTwoArrow(points, options);
    ...
    callback && callback(shapeTwoArrow);
};
// fabric.ShapeTwoArrow.fromObject = function (object, callback) {
//     const points = [object.x1, object.y1, object.x2, object.y2];
...
//     const newObject = new fabric.ShapeTwoArrow(arrowPoints, object);
//     newObject.set({
...
fabric.ShapeTwoArrow.generate = function (options) {
  return new fabric.ShapeTwoArrow([50, 50, 200, 200], {
    stroke: 'red',
    strokeWidth: 5,
    ...options
  });
...
const shapeRect = fabric.ShapeTwoArrow.generate({
  stroke: 'red',
  strokeWidth: 15
});
